...
export default function AdminClassesPage() {
  ...
  // Raw teacher availability for the selected teacher
  const [teacherAvailBlocks, setTeacherAvailBlocks] = useState([]);
  ...
  useEffect(() => {
    ...
    setTeacherAvailBlocks([]);
  }, [showCreateClass]);

  // Fetch raw availability when teacher is selected in create modal
  useEffect(() => {
    const teacherId = createSessionForm.teacher_id;
    if (!showCreateClass || !teacherId) {
      setTeacherAvailBlocks([]);
      return;
    }
    let cancelled = false;
    setTeacherAvailLoading(true);
    apiFetch(`/admin/teachers/${teacherId}/availability`)
      .then(async (res) => {
        if (cancelled) return;
        if (!res.ok) { setTeacherAvailBlocks([]); return; }
        const data = await res.json();
        setTeacherAvailBlocks(data.availability || []);
      })
      .catch(() => { if (!cancelled) setTeacherAvailBlocks([]); })
      .finally(() => { if (!cancelled) setTeacherAvailLoading(false); });
    return () => { cancelled = true; };
  }, [showCreateClass, createSessionForm.teacher_id]);
                  ...
                  <>
                    ...
                    {spw > 0 && (
                      <>
                        ...
                        {/* Teacher's raw availability blocks */}
                        {createSessionForm.teacher_id && (
                          <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                            <div className="text-xs font-semibold text-gray-700 mb-1.5 flex items-center gap-1.5">
                              <svg className="w-3.5 h-3.5 text-[#3FA9A6]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
                            ...
                            ) : teacherAvailBlocks.length === 0 ? (
                              <div className="text-xs text-amber-700 bg-amber-50 rounded-lg border border-amber-200 px-2.5 py-2">
                                ⚠ No availability set. The teacher needs to add their schedule first.
                              </div>
                            ) : (
                              <div className="flex flex-wrap gap-1.5">
                                {teacherAvailBlocks.map((b, i) => {
                                  ...
                                  return (
                                    <span key={i} className="inline-flex items-center gap-1 rounded-md bg-emerald-50 border border-emerald-200 px-2 py-1 text-[11px] font-medium text-emerald-800">
                                      {dayNames[b.day_of_week]} {fmtT(b.start_time)}–{fmtT(b.end_time)}
                                    </span>
                                  );
                                })}
                              </div>
